...
class Mobile_Stats {
    ...
    public function __construct($plugin)
    {
        ...
        try {
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[mobiles_list] " . $ex->getTraceAsString());
            $this->rc->output->send();
            return;
        }
    }
    ...
    public function init()
    {
        ...
        if (isset($fid)) {
            ...
            $this->rc->output->add_handlers(array(
                'melanie2_statistics_device_info'    => array($this, 'device_info'),
                'melanie2_statistics_sync_details'   => array($this, 'sync_details'),
            ));
            ...
            $this->rc->output->send('melanie2_moncompte.mobile_info');
        } else {
            // register UI objects
            $this->rc->output->add_handlers(array(
                'melanie2_statistics_mobiles_list'    => array($this, 'mobiles_list'),
                'melanie2_statistics_mobile_frame'    => array($this, 'mobile_frame'),
            ));
            $this->rc->output->include_script('list.js');
            $this->plugin->include_script('mobile.js');
            $this->rc->output->set_pagetitle($this->plugin->gettext('mobile'));
            $this->rc->output->send('melanie2_moncompte.mobile');
        }
    }
    ...
    public function mobiles_list($attrib)
    {
        ...
        $a_show_cols = array('melanie2_moncompte.deviceid', 'melanie2_moncompte.type', 'melanie2_moncompte.last_sync', 'melanie2_moncompte.z-push');
        ...
        try {
            ...
            foreach ($devices as $device) {
                ...
                $result[] = array(
                    ...
                    'melanie2_moncompte.deviceid' => $deviceId,
                    'melanie2_moncompte.type' => $devicetype,
                    'melanie2_moncompte.last_sync' => $lastsync,
                    'melanie2_moncompte.z-push' => $versionzpush,
                    'class' => '',
                );
        ...
        // set client env
        $this->rc->output->add_gui_object('melanie2_statistics_mobiles_list', $attrib['id']);
        ...
        try {
    		...
    		if (count($devices) > 0) {
        		...
        		$a_show_cols_accounts = array(
        		    'melanie2_moncompte.user',
        		    'melanie2_moncompte.first_sync',
        		    'melanie2_moncompte.last_sync',
        		    'melanie2_moncompte.z-push',
        		    'melanie2_moncompte.resync',
        		    'melanie2_moncompte.remove',
        		);
        		...
        		foreach ($devices as $device) {
        			...
        			if ($one) {
        				...
        				// define list of cols to be displayed
        				$a_show_cols = array(
        				    'melanie2_moncompte.deviceid',
        				    'melanie2_moncompte.imei',
        				    'melanie2_moncompte.devicetype',
        				    'melanie2_moncompte.devicemodel',
        				    'melanie2_moncompte.operator',
        				    'melanie2_moncompte.user-agent',
        				    'melanie2_moncompte.version',
        				    'melanie2_moncompte.language',
        				);

        				$result = array(array(
        				    'id' => $attrib['id'].$deviceId,
        				    'melanie2_moncompte.deviceid' => $deviceId,
        				    'melanie2_moncompte.imei' => $imei,
        				    'melanie2_moncompte.devicetype' => $devicetype,
        				    'melanie2_moncompte.devicemodel' => $devicemodel,
        				    'melanie2_moncompte.operator' => $operator,
        				    'melanie2_moncompte.user-agent' => $useragent,
        				    'melanie2_moncompte.version' => $deviceos,
        				    'melanie2_moncompte.language' => $deviceoslanguage,
        				    'class' => '',
        				));
        				...
        				if ($versionzpush >= 2) {
        				    ...
        				    // define list of cols to be displayed
        				    $a_show_cols = array(
        				        'melanie2_moncompte.asversion',
        				        'melanie2_moncompte.wipestatus',
        				        'melanie2_moncompte.wiperequestedby',
        				        'melanie2_moncompte.wiperequestedon',
        				        'melanie2_moncompte.wipeactionon',
        				    );

        				    $result = array(array(
        				        'id' => $attrib['id'].$deviceId,
        				        'melanie2_moncompte.asversion' => $asversion,
        				        'melanie2_moncompte.wipestatus' => $wipestatus,
        				        'melanie2_moncompte.wiperequestedby' => $wiperequestedby,
        				        'melanie2_moncompte.wiperequestedon' => $wiperequestedon,
        				        'melanie2_moncompte.wipeactionon' => $wipeactionon,
        				        'class' => '',
        				    ));
        			...
        			$result_accounts[] = array(
        			    ...
        			    'melanie2_moncompte.user' => $userDevice,
            		    'melanie2_moncompte.first_sync' => $firstsync,
            		    'melanie2_moncompte.last_sync' => $lastsync,
            		    'melanie2_moncompte.z-push' => $maxzpushvers,
        			    'melanie2_moncompte.resync' => "",
        			    'melanie2_moncompte.remove' => "",
        			    'class' => '',
    			    );
        		...
        		// create XHTML table
        		$out .= rcube_table_output($attrib, $result_accounts, $a_show_cols_accounts, 'id');
    		}
        }
        catch (Exception $ex) {
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[mobiles_list] " . $ex->getTraceAsString());
    ...
    public function sync_details($attrib) {
        ...
        try {
    		...
    		if (count($foldersinfos) > 0) {
        		// define list of cols to be displayed
        		$a_show_cols = array(
        		    'melanie2_moncompte.mailbox',
        		    'melanie2_moncompte.type',
        		    'melanie2_moncompte.foldersync',
        		    'melanie2_moncompte.last_sync',
        		    'melanie2_moncompte.z-push',        		    
        			'melanie2_moncompte.resync',
        			'melanie2_moncompte.remaining',
        		);
        		$result = array();

        		// Pour chaque folder : c'est à dire pour chaque backend
        		melanie2_logs::get_instance()->log(melanie2_logs::DEBUG, "[sync_details] " . print_r($foldersinfos, 1));
        			...
        			foreach ($folderinfo as $folderid => $zpush) {
        				foreach ($zpush as $zpushversion => $data) {
        					...
        					$result[] = array(
        					    ...
        					    'melanie2_moncompte.mailbox' => $userFolder,
        					    'melanie2_moncompte.type' => $type,
        					    'melanie2_moncompte.foldersync' => $folderidname,
        					    'melanie2_moncompte.last_sync' => $lastsync,
        					    'melanie2_moncompte.z-push' => $zpushversion,
        						'melanie2_moncompte.resync' => "",
        						'melanie2_moncompte.remaining' => $synced . " / " . $total,
    					    );
        				}
        			}
        		}
    ...
    public function zpush_command() {
        ...
        $command = trim(rcube_utils::get_input_value('_command', RCUBE_INPUT_POST));
        $deviceId = trim(rcube_utils::get_input_value('_device_id', RCUBE_INPUT_POST));
        $unlock = trim(rcube_utils::get_input_value('_unlock', RCUBE_INPUT_POST));
            ...
            if (isset($command)
                ...
                switch ($command) {
                    ...
                    case "ResyncFolderId":
                        ...
                        $folderId = trim(rcube_utils::get_input_value('_folder_id', RCUBE_INPUT_POST));
                        $userFolder = trim(rcube_utils::get_input_value('_user_folder', RCUBE_INPUT_POST));
                        if (isset($folderId)
                                && isset($userFolder)) {
                    ...
                    case "ResyncUserDevice":
                        ...
                        $userDevice = trim(rcube_utils::get_input_value('_user_device', RCUBE_INPUT_POST));
                        if (isset($userDevice)) {
