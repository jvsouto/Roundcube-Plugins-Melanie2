...
class valideAcces {
	...
	static function changeAcces() {
		...
		$case_internet = rcube_utils::get_input_value('chk_aiu', RCUBE_INPUT_POST);

		$info = array();
		if ( !empty($_POST['chk_aiu']) && $case_internet == '1') {
			$info['mineqMelAccesInternetU'] = 1;        // Activer l'Acces Internet U
			$message = rcmail::get_instance()->gettext('inter_acces_ok', 'melanie2_moncompte');
		} else {
			$info['mineqMelAccesInternetU'] = 0;        // Désactiver l'Acces Internet U
			$message = rcmail::get_instance()->gettext('inter_acces_nok', 'melanie2_moncompte');
		}
