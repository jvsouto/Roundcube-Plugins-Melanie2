...
class Moncompte {
		...
		if (isset($fid)) {
			...
			switch ($fid) {
				case 'rcmmodifmdp':
					...
					$this->rc->output->send('melanie2_moncompte.changepassword');
					break;
				...
				case 'rcminfoperso':
					...
					$this->rc->output->send('melanie2_moncompte.infoperso');
					break;
				...
				case 'rcmaccesinternet':
					...
					$this->rc->output->send('melanie2_moncompte.accesinternet');
					break;
				...
				case 'rcmphoto':
					...
					$this->rc->output->send('melanie2_moncompte.photo');
					break;
				...
				case 'rcmgestionabs':
					...
					$this->rc->output->send('melanie2_moncompte.absence');
					break;

				case 'rcmstatszp':
					$this->rc->output->set_pagetitle($this->plugin->gettext('moncompte'));
					$this->rc->output->add_handlers(array('statszp' => array($this, 'statsZP'),));
					$this->rc->output->send('melanie2_moncompte.zpstats');
					break;
				...
				case 'rcmgestionlists':
					...
					$this->rc->output->send('melanie2_moncompte.listes');
					break;
				default:
				case '':
					$this->rc->output->set_pagetitle($this->plugin->gettext('moncompte'));
			...
			// register UI objects
			$this->rc->output->add_handlers(array(
					'melanie2_moncompte_options_list'    => array($this, 'options_list'),
					'melanie2_moncompte_option_frame'    => array($this, 'option_frame'),
			));
			$this->rc->output->include_script('list.js');
			$this->rc->output->set_pagetitle($this->plugin->gettext('moncompte'));
			$this->rc->output->send('melanie2_moncompte.moncompte');
		}
	}
	...
	public function options_list($attrib) {
		...
		// set client env
		$this->rc->output->add_gui_object('melanie2_moncompte_options_list', $attrib['id']);
	...
	// CGU pour DDI
			...
			$user = melanie2::get_user_infos(Moncompte::get_current_user_name());
			$userDN = $user['dn'];
...
// ---------------  
	...
	public function readUserListes($attrib) {
		...
		if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, var_export($liste_listes, true));
	...
	public function readListeMembers() {
		...
		&& $this->isOwnerListe($dn_list)) {
			...
			if (count($res) == 2) {
				$base_dn = $res[1];
				$filter = $res[0];
				$ldap_attrs = array('mineqmelmembres');
		...
		sort($liste_members);
		$result = array('action' => 'plugin.listes_membres', 'data' => $liste_members, 'dn_list' => $dn_list, 'unlock' => $unlock);
		if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, var_export($liste_members, true));
	...
	public function addExterneMember() {
		$dn_list = rcube_utils::get_input_value('_dn_list', RCUBE_INPUT_POST);
		$new_smtp = rcube_utils::get_input_value('_new_smtp', RCUBE_INPUT_POST);
		$unlock = rcube_utils::get_input_value('_unlock', RCUBE_INPUT_POST);
				...
				if (!in_array(strtolower($new_smtp), $liste_members)) {
					if (Ldap::GetInstance(Config::$MASTER_LDAP)->authenticate($this->rc->config->get('liste_admin'), $this->rc->config->get('liste_pwd'))) {
						...
						if (!Ldap::GetInstance(Config::$MASTER_LDAP)->modify($dn_list, array('mineqmelmembres' => $liste_members, 'memberuid' => $liste_uid))) {
							melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "moncompte::addExterneMember() ldap_modify ERROR : " . Ldap::GetInstance(Config::$MASTER_LDAP)->getError());
							melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "moncompte::addExterneMember() ldap_modify DN : " . $dn_list);
							melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "moncompte::addExterneMember() ldap_modify OBJECT : " . var_export(array('mineqmelmembres' => $liste_members, 'memberuid' => $liste_uid), true));
						}
					}
				}
			}
	...
	public function RemoveMember(){
		$dn_list = rcube_utils::get_input_value('_dn_list', RCUBE_INPUT_POST);
		$address = rcube_utils::get_input_value('_address', RCUBE_INPUT_POST);
		$unlock = rcube_utils::get_input_value('_unlock', RCUBE_INPUT_POST);
		...
		&& $this->isOwnerListe($dn_list)) {
			...
			if (count($res) == 2) {
				...
				if (Ldap::GetInstance(Config::$MASTER_LDAP)->authenticate($this->rc->config->get('liste_admin'), $this->rc->config->get('liste_pwd'))) {
					...
					if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, Ldap::GetInstance(Config::$MASTER_LDAP)->getError());
				}
			}
		}
		$liste_members = array_map('strtolower', $liste_members);
	...
	public function ExportMembers() {
		if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "[ExportMembers] ".var_export($_GET, true));

		$dn_list = rcube_utils::get_input_value('_dn_list', RCUBE_INPUT_GET);
		$export = "";
		if (isset($dn_list)
		&& $this->isOwnerListe($dn_list)) {
			$res = explode(',', $dn_list, 2);
	...
	private function isOwnerListe($dn_list) {
		...
		if (count($res) == 2) {
			...
			$user = melanie2::get_user_infos(Moncompte::get_current_user_name());
			$userDN = $user['dn'];
			if (Ldap::GetInstance(Config::$AUTH_LDAP)->authenticate($userDN, $this->rc->get_user_password())) {
	...
	public function moncompte_balp_list($attrib) {
		...
		$balp_list = melanie2::get_user_balp_gestionnaire($this->rc->get_user_name());
		...
		$infos = melanie2::get_user_infos($this->rc->get_user_name());
		...
		if (is_array($balp_list)) {
			foreach($balp_list as $balp) {
				...
				$infos = melanie2::get_user_infos($name);
	...
	// 		$this->rc->output->show_message('melanie2_moncompte.changepassword_confirm', 'confirmation');
	// 	}
