...
// 		}
		...
		if (!empty($codepos) && strlen($codepos) !== 5) {
			...
			rcmail::get_instance()->output->show_message('melanie2_moncompte.info_err_postal', 'error');
		...
		if(Ldap::GetInstance(Config::$MASTER_LDAP)->modify($user_dn, $infosperso)) {
			rcmail::get_instance()->output->show_message('melanie2_moncompte.info_modif_ok', 'confirmation');
		} else {
			$err = Ldap::GetInstance(Config::$MASTER_LDAP)->getError();
			rcmail::get_instance()->output->show_message('melanie2_moncompte.info_modif_nok' . $err, 'error');
		}
	}
	...
	function format_tel($numero, $type) {
		...
		if ((preg_match('/^\d*$/',$numero) > 0) && ($l > 9) && ($l < 12)) {
			...
			rcmail::get_instance()->output->show_message('melanie2_moncompte.info_err_tel' . $type, 'error');
			return '';
		}
	}		
}
