...
class valideCGUMob {
	...
	static function changeCGUMob() {
		...
		$case_cgu_mob = rcube_utils::get_input_value('chk_cgu_mob', RCUBE_INPUT_POST);
		...
		if ( !empty($case_cgu_mob)) {		
			...
			if(Ldap::GetInstance(Config::$MASTER_LDAP)->modify($user_dn, $info)) {
				rcmail::get_instance()->output->show_message(rcmail::get_instance()->gettext('inter_CGUmob_success', 'melanie2_moncompte'), 'confirmation');
			} else {
				$err = Ldap::GetInstance(Config::$MASTER_LDAP)->getError();
				rcmail::get_instance()->output->show_message(rcmail::get_instance()->gettext('inter_CGUmob_error', 'melanie2_moncompte') . $err, 'error');
			}
		} else {
			rcmail::get_instance()->output->show_message(rcmail::get_instance()->gettext('inter_CGUmob_fake', 'melanie2_moncompte'), 'error');
		}
	}
}
