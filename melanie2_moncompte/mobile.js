...
}, "tbody td.melanie2_moncompte\\.resync"); // pass the element as an argument to .on
...
}, "tbody td.melanie2_moncompte\\.remove"); // pass the element as an argument to .on

$(document).on({
    click: function (e) {
    	switch($(this).attr('id')) {
	    	case "melanie2_statistics_delete_device":
	    		var param = {
	    			_command: 'Delete',
	    			_device_id: rcmail.env.deviceid,
	    		};
	    		break;
	    	case "melanie2_statistics_deletezp1_device":
	    		var param = {
	    			_command: 'DeleteZP1',
	    			_device_id: rcmail.env.deviceid,
	    		};
	    		break;
	    	case "melanie2_statistics_resync_device":
	    		var param = {
	    			_command: 'Resync',
	    			_device_id: rcmail.env.deviceid,
	    		};
	    		break;
	    	case "melanie2_statistics_wipe_device":
	    		var param = {
	    			_command: 'Wipe',
	    			_device_id: rcmail.env.deviceid,
	    		};
	    		break;
	    	case "melanie2_statistics_unwipe_device":
	    		var param = {
	    			_command: 'Unwipe',
	    			_device_id: rcmail.env.deviceid,
	    		};	    		
...
if (window.rcmail) {
	...
	rcmail.addEventListener('init', function(evt) {
		var p = rcmail;
		if (rcmail.gui_objects.melanie2_statistics_mobiles_list) {
	        rcmail.mobiles_list = new rcube_list_widget(rcmail.gui_objects.melanie2_statistics_mobiles_list,
	          {multiselect:false, draggable:false, keyboard:true});
	        rcmail.mobiles_list.addEventListener('select', function(e) { p.melanie2_statistics_mobile_select(e); });
		...
		if ($("#rcmaccountsinfo").length > 0) {
			$("#rcmaccountsinfo tbody tr").each(function() {
				var td = $(this).children('.melanie2_moncompte\\.z-push');
				if (td.html() == '1') {
					$(this).children('.melanie2_moncompte\\.resync').removeClass('melanie2_moncompte.resync');					
				}
				var td_user = $(this).children('.melanie2_moncompte\\.user');
				if (td_user.html().indexOf(rcmail.env.current_user) == -1) {
					$(this).children('.melanie2_moncompte\\.resync').removeClass('melanie2_moncompte.resync');
					$(this).children('.melanie2_moncompte\\.remove').removeClass('melanie2_moncompte.remove');
				}
			});			
		}
		if ($("#melanie2_statistics_sync_details").length > 0) {			
			$("#melanie2_statistics_sync_details tbody tr").each(function() {
				var td = $(this).children('.melanie2_moncompte\\.z-push');
				if (td.html() == '1') {
					$(this).children('.melanie2_moncompte\\.resync').removeClass('melanie2_moncompte.resync');
				}
				var td_user = $(this).children('.melanie2_moncompte\\.mailbox');
				if (td_user.html().indexOf(rcmail.env.current_user) == -1) {
					$(this).children('.melanie2_moncompte\\.resync').removeClass('melanie2_moncompte.resync');
				}
			});			
...
// Mobile selection
rcube_webmail.prototype.melanie2_statistics_mobile_select = function(option)
{
...
rcube_webmail.prototype.load_mobile_frame = function(id)
{
  ...
  if (this.env.contentframe && window.frames && window.frames[this.env.contentframe]) {
    ...
    target.location.href = this.env.comm_path+'&_action=plugin.melanie2_statistics_mobile&_framed=1'
...
function zpush_send_command(param) {
	if (confirm(rcmail.gettext("melanie2_moncompte.confirm_" + param._command))) {
		...
		var lock = rcmail.display_message(rcmail.gettext('melanie2_moncompte.wait'), 'loading');
