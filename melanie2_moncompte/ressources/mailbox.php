...
class M2mailbox {
  ...
  public function getAcl() {
    ...
    $infos = LibMelanie\Ldap\Ldap::GetUserInfos($id, null, array('mineqmelpartages'), LibMelanie\Config\Ldap::$SEARCH_LDAP);
  ...
  public function setAcl($user, $rights) {
    ...
    $_infos = melanie2::get_user_infos($user);
    if (! isset($_infos)) {
      return false;
    }
    $haschanged = false;
  ...
  public function resources_elements_list($attrib) {
    ...
    $balp = melanie2::get_user_balp($this->user->uid);
    if (! isset($balp)) {
      $balp = array();
    }
    else {
  ...
  public function acl_frame($attrib) {
    ...
    $attrib['src'] = $this->rc->url(array('_action' => 'plugin.melanie2_mailbox_acl','id' => $id,'framed' => 1));
    $attrib['width'] = '100%';
    $attrib['height'] = 275;
    $attrib['border'] = 0;
  ...
  public function restore_bal($attrib) {
    ...
    foreach ($folders as $folder) {
      ...
      if (strtoupper($name) == 'INBOX') {
        $name = $this->rc->gettext(strtoupper($name), 'melanie2_moncompte');
      }
    ...
    $html = html::div(null, str_replace('%%NB_HEURES%%', $input->show(), $this->rc->gettext('imap_select', 'melanie2_moncompte')) . $select->show());
    return $html;
  }
  ...
  public static function unexpunge() {
    ...
    if (! empty($nbheures)) {
      ...
      if (isset($infos) && isset($infos['mineqmelroutage']) && count($infos['mineqmelroutage']) > 0) {
        ...
        rcmail::get_instance()->output->show_message('melanie2_moncompte.restore_bal_succes', 'confirmation');
      }
      else {
        rcmail::get_instance()->output->show_message('melanie2_moncompte.restore_bal_error', 'error');
      }
    }
  }
}
