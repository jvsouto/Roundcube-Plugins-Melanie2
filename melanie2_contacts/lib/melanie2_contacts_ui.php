...
class melanie2_contacts_ui
{
     ...
     * Class constructor
     *
     * @param melanie2_contacts $plugin Plugin object
     */
    ...
    private function init_ui()
    {
        ...
        // Include script
        $this->plugin->include_script('melanie2_contacts.js');

        if (empty($this->rc->action)) {
            // Include stylesheet (for directorylist)
            $this->plugin->include_stylesheet($this->plugin->local_skin_path().'/melanie2_contacts.css');
            ...
            foreach ($options as $command) {
                $content = html::tag('li', $idx ? null : array('class' => 'separator_above'),
                    $this->plugin->api->output->button(array(
                        'label'    => 'melanie2_contacts.'.str_replace('-', '', $command),
                        'domain'   => $this->ID,
                        'classact' => 'active',
                        'command'  => $command
                )));
            ...
            $content = html::tag('li', array('class' => 'separator_above'),
                    $this->plugin->api->output->button(array(
                        'label'    => 'melanie2_contacts.manageresources',
                        'type'     => 'link',
                        'classact' => 'active',
                        'command'  => 'plugin.melanie2_resources_contacts',
                        'task'     => 'settings',
                    )));
            $this->plugin->api->add_content($content, 'groupoptions');

            $this->rc->output->add_label('melanie2_contacts.bookdeleteconfirm',
                'melanie2_contacts.bookdeleting', 'melanie2_contacts.bookshowurl',
                'melanie2_contacts.carddavurldescription');
        }
        // book create/edit form
        else {
            $this->rc->output->add_label('melanie2_contacts.nobooknamewarning',
                'melanie2_contacts.booksaving');
        }
    }
    ...
    public function book_edit()
    {
        ...
        $this->rc->output->send('melanie2_contacts.bookedit');
    }
