...
require_once('melanie2_contacts_mapping.php');

class melanie2_addressbook extends rcube_addressbook
{
    /**
     * @var rcube
     */
    ...
    */
    ...
    function list_records($cols=null, $subset=0) {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::list_records($cols, $subset)");
        ...
        try {
                ...
                if ($cache['contacts'][$this->addressbook->id]['ctag'] == $this->ctag) {
                    ...
                    if (isset($cache['contacts'][$this->addressbook->id]['list'][$limit."_".$offset])
                        ...
                        foreach ($cache['contacts'][$this->addressbook->id]['list'][$limit."_".$offset] as $_contact) {
                            ...
                            $this->result->add(melanie2_contacts_mapping::m2_to_rc_contact($cols, $_contact));
            ...
            if (isset($this->group_id)) {
                ...
                if (!isset($cols)) $cols = array_keys(melanie2_contacts_mapping::$mapping_contact_cols);
                $fields = array();
                foreach($cols as $col) {
                    $fields[] = melanie2_contacts_mapping::$mapping_contact_cols[$col];
                }
                ...
                if (is_array($members) && count($members) > 0) {
                    ...
                    // MANTIS 3839: Le nombre de membres par groupe n'est pas toujours bon
                    $members = array_unique($members);
                    $old_members = $members;
                    foreach ($_contacts as $_contact) {
                      if (($key = array_search($_contact->id, $old_members)) !== false) {
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::list_records() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    */
    function search($fields, $value, $mode=0, $select=true, $nocount=false, $required=array()) {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::search($fields, $value, $mode, $select, $nocount, $required");
        try {
            $_contact = new Melanie2\Contact($this->user, $this->addressbook);
            $_contact->type = Melanie2\Contact::TYPE_CONTACT;
            $filter = "(";
            ...
            foreach($fields as $key => $field) {
                if (isset(melanie2_contacts_mapping::$mapping_contact_cols[$field])) {
                    $mapfield = melanie2_contacts_mapping::$mapping_contact_cols[$field];
                    if ($filter != "(") $filter .= " OR ";
                    $filter .= "#$mapfield#";
            ...
            foreach ($_contact->getList($searchfields, $filter, $operators, "object_lastname, object_firstname", true, null, null, $case_unsensitive_fields) as $_contact) {
                $this->result->add(melanie2_contacts_mapping::m2_to_rc_contact(null, $_contact));
                $i++;
            }
            $this->result->count = $i;
            return $this->result;
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::search() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    */
    function count() {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::count()");
        if (isset($this->group_id)) {
          ...
          try {
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::count() Melanie2DatabaseException");
            return false;
          }
          catch (\Exception $ex) {
            return false;
    ...
    */
    function get_record($id, $assoc=false) {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::get_record($id, $assoc)");
        try {
            // Chargement du contact depuis le cache
            $cache = \melanie2::InitM2Cache();
            $limit = $this->page_size;
            ...
            $this->result = new rcube_result_set();
            $this->result->add(melanie2_contacts_mapping::m2_to_rc_contact(null, $contact));
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::get_record() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::insert($save_data, $check)");
        if ($this->readonly) {
            return false;
        }
        try {
            $contact = melanie2_contacts_mapping::rc_to_m2_contact($save_data, new Melanie2\Contact($this->user, $this->addressbook));
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::insert() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::update($id, $save_cols)");
        if ($this->readonly) {
            return false;
        }
        try {
            ...
            if (isset($contact)) {
                $contact = melanie2_contacts_mapping::rc_to_m2_contact($save_cols, $contact);
                $contact->id = $id;
                $contact->modified = time();
                $ret = $contact->save();
                // Chargement du contact depuis le cache
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::update() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::delete($ids, $force)");
        if ($this->readonly) {
            return false;
        }
        try {
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::delete() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::delete_all($with_groups)");
        if ($this->readonly) {
            return false;
        }
        try {
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::delete_all() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::get_group($group_id)");
        try {
            // Chargement du groupe depuis le cache
            $cache = \melanie2::InitM2Cache();
            if (isset($cache['contacts'])
                ...
                return melanie2_contacts_mapping::m2_to_rc_contact(null, $group);
            } else {
                $_group = new Melanie2\Contact($this->user, $this->addressbook);
                $_group->type = Melanie2\Contact::TYPE_LIST;
                $_group->id = $group_id;
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::get_group() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::create_group($name)");
        try {
            $_group = new Melanie2\Contact($this->user, $this->addressbook);
            $_group->type = Melanie2\Contact::TYPE_LIST;
            ...
            if (!is_null($ret)) {
                ...
                $group = melanie2_contacts_mapping::m2_to_rc_contact(null, $_group);
                $group['id'] = $_group->id;
                return $group;
            } else return false;
        }
        catch (LibMelanie\Exceptions\Melanie2DatabaseException $ex) {
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::create_group() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::delete_group($gid)");
        try {
            $_group = new Melanie2\Contact($this->user, $this->addressbook);
            $_group->type = Melanie2\Contact::TYPE_LIST;
            $_group->id = $gid;
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::delete_group() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::rename_group($gid, $newname, &$newid)");
        try {
            $_group = new Melanie2\Contact($this->user, $this->addressbook);
            $_group->type = Melanie2\Contact::TYPE_LIST;
            $_group->id = $gid;
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::rename_group() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::add_to_group($group_id, $ids)");
        try {
            $_group = new Melanie2\Contact($this->user, $this->addressbook);
            $_group->type = Melanie2\Contact::TYPE_LIST;
            $_group->id = $group_id;
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::add_to_group() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
    ...
    {
        if (melanie2_logs::is(melanie2_logs::TRACE)) melanie2_logs::get_instance()->log(melanie2_logs::TRACE, "melanie2_addressbook::remove_from_group($group_id, $ids)");
        try {
            $_group = new Melanie2\Contact($this->user, $this->addressbook);
            $_group->type = Melanie2\Contact::TYPE_LIST;
            $_group->id = $group_id;
            ...
            melanie2_logs::get_instance()->log(melanie2_logs::ERROR, "[calendar] melanie2_addressbook::remove_from_group() Melanie2DatabaseException");
            return false;
        }
        catch (\Exception $ex) {
            return false;
