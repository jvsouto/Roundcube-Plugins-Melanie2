...
rcube_webmail.prototype.set_book_actions = function()
{
    ...
    this.enable_command('book-edit', 'book-delete', source && sources[source] && sources[source].melanie2 && sources[source].editable);
...
{
    if (this.env.source != '' && confirm(this.get_label('melanie2_contacts.bookdeleteconfirm'))) {
        var lock = this.set_busy(true, 'melanie2_contacts.bookdeleting');
...
rcube_webmail.prototype.book_showurl = function()
{
    ...
    if (source && source.carddavurl) {
        ...
        var $dialog = $('<div>').addClass('showurldialog').append('<p>'+rcmail.gettext('carddavurldescription', 'melanie2_contacts')+'</p>'),
            textbox = $('<textarea>').addClass('urlbox').css('width', '100%').attr('rows', 2).appendTo($dialog);

          $dialog.dialog({
            resizable: true,
            closeOnEscape: true,
            title: rcmail.gettext('bookshowurl', 'melanie2_contacts'),
            close: function() {
...
rcube_webmail.prototype.book_save = function()
{
    ...
    if (input.length && input.val() == '') {
        alert(this.get_label('melanie2_contacts.nobooknamewarning'));
        input.focus();
        return;
    }

    input = this.display_message(this.get_label('melanie2_contacts.booksaving'), 'loading');
