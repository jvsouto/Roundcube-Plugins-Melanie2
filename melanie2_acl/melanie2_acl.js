...
rcube_webmail.prototype.acl_delete = function()
{
    ...
    if (users && users.length && confirm(this.get_label('melanie2_acl.deleteconfirm'))) {
        this.http_post('settings/plugin.acl', {
            ...
            this.set_busy(true, 'melanie2_acl.deleting'));
    }
}
...
rcube_webmail.prototype.acl_save = function()
{
    ...
    if (!user) {
        alert(this.get_label('melanie2_acl.nouser'));
        return;
    }
    if (!rights) {
        alert(this.get_label('melanie2_acl.norights'));
        return;
    }
    ...
    this.http_post('settings/plugin.acl', data, this.set_busy(true, 'melanie2_acl.saving'));
}
...
rcube_webmail.prototype.acl_init_form = function(id)
{
    ...
    this.acl_popup = this.show_popup_dialog(
        popup_wrapper,
        id ? this.gettext('melanie2_acl.editperms') : this.gettext('melanie2_acl.newuser'),
        buttons,
        {
