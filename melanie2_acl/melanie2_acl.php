...
class melanie2_acl extends rcube_plugin
{
    ...
    function folder_form($args)
    {
        ...
        $this->include_script('melanie2_acl.js');
        $this->rc->output->include_script('list.js');
        $this->include_stylesheet($this->local_skin_path().'/melanie2_acl.css');
        ...
        $args['form']['sharing'] = array(
            ...
            'content' => $this->rc->output->parse('melanie2_acl.table', false, false),
        );
...
//             $username = $this->mod_login($username);
        ...
        if ($result) {
            $this->rc->output->show_message($oldid ? 'melanie2_acl.updatesuccess' : 'melanie2_acl.createsuccess', 'confirmation');
        }
        else {
            $this->rc->output->show_message($oldid ? 'melanie2_acl.updateerror' : 'melanie2_acl.createerror', 'error');
        }
    }
    ...
    private function action_delete()
    {
        ...
        if (!$error) {
            $this->rc->output->show_message('melanie2_acl.deletesuccess', 'confirmation');
        }
        else {
            $this->rc->output->show_message('melanie2_acl.deleteerror', 'error');
        }
    }
