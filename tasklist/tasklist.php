...
class tasklist extends rcube_plugin
{
    ...
    {
        $filter = intval(rcube_utils::get_input_value('filter', RCUBE_INPUT_GPC));
        $action = rcube_utils::get_input_value('action', RCUBE_INPUT_GPC);
        $rec  = rcube_utils::get_input_value('t', RCUBE_INPUT_POST, true);
        $oldrec = $rec;
        ...
        case 'delete':
            $mode  = intval(rcube_utils::get_input_value('mode', RCUBE_INPUT_POST));
        ...
        case 'collapse':
            foreach (explode(',', $rec['id']) as $rec_id) {
                if (intval(rcube_utils::get_input_value('collapsed', RCUBE_INPUT_GPC))) {
    ...
    {
        $action = rcube_utils::get_input_value('action', RCUBE_INPUT_GPC);
        $list  = rcube_utils::get_input_value('l', RCUBE_INPUT_POST, true);
        $success = false;
    ...
    public function fetch_counts()
    {
        if (isset($_REQUEST['lists'])) {
            $lists = rcube_utils::get_input_value('lists', RCUBE_INPUT_GPC);
        }
        else {
    ...
    {
        $f = intval(rcube_utils::get_input_value('filter', RCUBE_INPUT_GPC));
        $search = rcube_utils::get_input_value('q', RCUBE_INPUT_GPC);
...
//            "rcmail.set_env('loadingicon', '" . $this->api->output->env['loadingicon'] . "');\n".
    ...
    public function refresh($attr)
    {
        ...
        $filter = array(
            'since'  => $attr['last'],
            'search' => rcube_utils::get_input_value('q', RCUBE_INPUT_GPC),
            'mask'   => intval(rcube_utils::get_input_value('filter', RCUBE_INPUT_GPC)) & self::FILTER_MASK_COMPLETE,
        );
        $lists = rcube_utils::get_input_value('lists', RCUBE_INPUT_GPC);;
    ...
    */
    ...
    public function attachment_get()
    {
        ...
        $task = rcube_utils::get_input_value('_t', RCUBE_INPUT_GPC);
